...
export class PizzaData extends BaseDatabase {
    ...
    createIngredient = async (data:Ingredient) => {
        try {
            await PizzaData
            .connection(`Ambulnz_Ingredientes`)
            .insert({
                id: data.getId(),
                name: data.getName()
            });
    ...
    addIngredientsToPizza = async (pizza_id:string, ingredient_id:string) => {
        try {
            await PizzaData
            .connection(`Ambulnz_Pizza_Ingredientes`)
            .insert({
                pizza_id,
                ingredient_id
            });
    ...
    joinIngredientsAndPizza =async (pizza_id:string) => {
        try {
            const ingredients = await PizzaData
            .connection
            .select(`Ambulnz_Ingredientes.name`)
            .from(`Ambulnz_Pizza_Ingredientes`)
            .innerJoin(`Ambulnz_Pizza`, `Ambulnz_Pizza.id`, `Ambulnz_Pizza_Ingredientes.pizza_id`)
            .innerJoin(`Ambulnz_Ingredientes`, `Ambulnz_Ingredientes.id`, `Ambulnz_Pizza_Ingredientes.ingredient_id`)
